...
public class UIPlayAnimation : MonoBehaviour
{
	...
	bool dualState { get { return trigger == AnimationOrTween.Trigger.OnPress || trigger == AnimationOrTween.Trigger.OnHover; } }
	...
	void OnEnable ()
	{
		...
		{
			if (trigger == AnimationOrTween.Trigger.OnPress || trigger == AnimationOrTween.Trigger.OnPressTrue)
				mActivated = (UICamera.currentTouch.pressed == gameObject);

			if (trigger == AnimationOrTween.Trigger.OnHover || trigger == AnimationOrTween.Trigger.OnHoverTrue)
	...
	void OnClick () { if (enabled && trigger == AnimationOrTween.Trigger.OnClick) Play(true, false); }

	void OnDoubleClick () { if (enabled && trigger == AnimationOrTween.Trigger.OnDoubleClick) Play(true, false); }
	...
	void OnToggle ()
	{
		...
		if (trigger == AnimationOrTween.Trigger.OnActivate ||
		    (trigger == AnimationOrTween.Trigger.OnActivateTrue && UIToggle.current.value) ||
		    (trigger == AnimationOrTween.Trigger.OnActivateFalse && !UIToggle.current.value))
	...
	void OnDragOver ()
	{
		if (enabled && dualState)
		{
			...
			else if (dragHighlight && trigger == AnimationOrTween.Trigger.OnPress) Play(true, true);
		}
	}
	...
	{
		if (enabled && trigger == AnimationOrTween.Trigger.OnPress && UICamera.currentTouch.dragged != gameObject)
			Play(false, true);
	}
